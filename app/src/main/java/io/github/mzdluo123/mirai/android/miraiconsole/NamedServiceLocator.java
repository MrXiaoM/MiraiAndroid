 ...
 *
 */
public final class NamedServiceLocator
        implements ServiceLocator
{
    ...
    private class Entry<T>
    {
        ...
        private T newInstance( Class<?> impl )
        {
            try
            {
                ...
                {
                    ( (Service) instance ).initService( NamedServiceLocator.this );
                }
                return instance;
            }
    ...
    private <T> Entry<T> getEntry(Class<T> type, boolean create )
    {
        ...
        Entry<T> entry = (Entry<T>) entries.get( requireNonNull( type, "service type cannot be null" ).getName() );
        if ( entry == null && create )
        {
            entry = new Entry<>( type );
            entries.put( type.getName(), entry );
        }
        return entry;
    }
     ...
     */
    public <T> NamedServiceLocator setService(Class<T> type, Class<? extends T> impl )
    {
     ...
     * @return This locator for chaining, never {@code null}.
     */
    public <T> NamedServiceLocator addService(Class<T> type, Class<? extends T> impl )
    {
     ...
     * @return This locator for chaining, never {@code null}.
     */
    public <T> NamedServiceLocator setServices(Class<T> type, T... services )
    {
     ...
     */
    public void setErrorHandler( org.eclipse.aether.impl.DefaultServiceLocator.ErrorHandler errorHandler )
    {
